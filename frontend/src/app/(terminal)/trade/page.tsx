...
export default async function TradePage({
  ...
  try {
    // Liveness comes from the shared guard, never the raw cached snapshot —
    // see liveMarketSnapshot() for why the cache cannot be trusted for this.
    const snap = await liveMarketSnapshot();
    // Re-derived against the clock: the terminal renders this list as
    // "routable", and a 10s-stale 1m window is already dead. The count in the
    // header has to mean what it says.
