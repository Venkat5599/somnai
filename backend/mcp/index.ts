...
/**
 * Built per call so each transport gets its own server instance.
 *
 * The POLICY and its ledger stay module-level on purpose: one process is one
 * budget. Two HTTP clients hitting the same host share the spend cap rather
 * than each getting a fresh one, which is the safe direction — the alternative
 * is a hosted endpoint where budget scales with the number of connections.
 */
...
/**
 * Stdio is the default: it is how Claude Desktop connects, and it needs no
 * network surface at all. `backend/mcp/http.ts` is the hosted variant.
 */
